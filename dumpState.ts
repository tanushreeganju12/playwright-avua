...
(async () => {
  ...
  // Click Hybrid
  await page.getByRole('heading', { name: /Hybrid/i }).first().click();
  ...
  console.log("React State for Hybrid:");
  console.log(JSON.stringify(state, null, 2));

  // Click Remote
  await page.getByRole('heading', { name: /Remote/i }).first().click();
  await page.waitForTimeout(1000);

  const stateRemote = await page.evaluate(() => {
    const candidates = Array.from(document.querySelectorAll('input, textarea, button, div, p, span, main'));
    let formStateHook: any = null;
    for (const el of candidates) {
      const key = Object.keys(el).find(k => k.startsWith('__reactFiber$'));
  ...
  console.log("React State for Remote:");
  console.log(JSON.stringify(stateRemote, null, 2));
