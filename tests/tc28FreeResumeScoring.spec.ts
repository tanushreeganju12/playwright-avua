...
test('TC28 - Applicant signup, verify magic link, select full-time job and verify CV Upload redirect', async ({ page }) => {
  ...
  const resumePath = `./fixtures/resume_tc28_${count}.pdf`;
  ...
  try {
    ...
    // Step 14: Click Free Resume Scoring and verify redirect to /cv-upload
    console.log('Clicking Free Resume Scoring button and verifying redirect...');
    await resumeScoringBtn.click();
    await page.waitForTimeout(6000);

    const url = page.url();
    console.log(`Current page URL after click: ${url}`);
    
    // Assert that URL redirects to cv-upload
    expect(url).toContain('/cv-upload');
    await page.screenshot({ path: `screenshots/tc28_cv_upload_redirect.png`, fullPage: true });
    console.log('Successfully verified redirect to CV Upload screen!');
