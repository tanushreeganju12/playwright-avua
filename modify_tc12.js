...
// Create the new TC12 content
const newTC12 = `  test('TC12 Successful submission with INDEPENDENT CONTRACTOR (IC) engagement model', async ({ page }) => {
    ...
    const icOption = page.getByText('Independent contractor (IC)', { exact: false }).first();
    if (await icOption.isVisible()) {
      await icOption.click();
      await page.waitForTimeout(1000);
    } else {
      // Sometimes it's IC only or different casing
      await page.locator('div', { hasText: 'Independent contractor' }).last().click({ force: true });
      await page.waitForTimeout(1000);
    }
...
console.log('Successfully updated tests/employerJobPost.spec.ts with TC12');
